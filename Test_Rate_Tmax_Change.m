...
%  Simparam.phi = pi*(0.2*rand(Sysparam.L,1)+0.3);%AoD;
% %Simparam.phi = spatialDirection;
...
Rate1 = zeros(Sysparam.K,length(TMax)); % ideal Analog precoder
Rate2 = zeros(Sysparam.K,length(TMax)); % joint TTD and PS 
Rate3 = zeros(Sysparam.K,length(TMax)); % fixed PS 
Rate4 = zeros(Sysparam.K,length(TMax)); % Dynamic subarray
Nrlz = 100; %number of channel realizations
Nloop = 20;
%% 
for n =1:length(TMax)
Sysparam.tmax = TMax(n);
Sysparam.T = Sysparam.tmax*(0:1:Sysparam.Q-1)/(Sysparam.Q-1);
