...
Rate1 = zeros(1,length(TMax)); %joint delay and phase precoding
Rate2 = zeros(1,length(TMax)); %fixed PS and design TTD 
Rate3 = zeros(1,length(TMax)); % fixed TTD and design switch network
Rate4 = zeros(1,length(TMax));
Rate5 = zeros(1,length(TMax));
% for k =1:Sysparam.K
%     Gain1(k) = 
% end 
%% 
Nrlz = 10; %channel realizationstic
...
for n = 1:length(TMax)
    ...
    Sysparam.tmax = TMax(n);% maximum time-delay value 
...
plot(TMax*1e+12,Rate1,'->r',TMax*1e+12,Rate2,'-*b',TMax*1e+12,Rate3,'-.k',TMax*1e+12,Rate4,'-o',TMax*1e+12,Rate5,'-<')
grid on 
