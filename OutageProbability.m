...
Sysparam.tmax = 340e-12;% maximum time-delay value  
...
Rate1 = zeros(1,length(NTTD)); %joint delay and phase precoding
Rate2 = zeros(1,length(NTTD)); %fixed PS and design TTD 
Rate3 = zeros(1,length(NTTD)); % fixed TTD and design switch network
Rate4 = zeros(1,length(NTTD));
Rate5 = zeros(1,length(NTTD));
Nloop = 20;
% for k =1:Sysparam.K
%     Gain1(k) = 
% end 
%% 
Nrlz = 30; %channel realizations
tic
for n = 1:length(NTTD)
...
for l =1:Nrlz
    ...
    for k = 1:Sysparam.K
        %% Fully digital optimal rate -Rate 1
          [~,~,V(:,:,k)] = svd(H(:,:,k));
          Rate1NK(k,n) = log2(det(eye(Sysparam.Ns)+(Simparam.Rho/Sysparam.Ns)*H(:,:,k)*V(:,1:Sysparam.Ns,k)*V(:,1:Sysparam.Ns,k)'*H(:,:,k)'));
          Rate1(n) = Rate1(n)+Rate1NK(k,n);
        %% Joint PS and TTD rate - Rate 2   
          Heff2(:,:,k) = H(:,:,k)*PS*TTD(:,:,k);
          [~,~,Veff2(:,:,k)] = svd(Heff2(:,:,k));
          Rate2NK(k,n) = log2(det(eye(Sysparam.Ns)+(Simparam.Rho/Sysparam.Ns)*Heff2(:,:,k)*Veff2(:,1:Sysparam.Ns,k)*Veff2(:,1:Sysparam.Ns,k)'*Heff2(:,:,k)'));
          Rate2(n) = Rate2(n)+Rate2NK(k,n);
         %% Fixed PS and design TTD rate - Rate 3
         Heff3(:,:,k) = H(:,:,k)*G1*G2k(:,:,k);
         [~,~,Veff3(:,:,k)] = svd(Heff3(:,:,k));
         Rate3NK(k,n) = log2(det(eye(Sysparam.Ns)+(Simparam.Rho/Sysparam.Ns)*Heff3(:,:,k)*Veff3(:,1:Sysparam.Ns,k)*Veff3(:,1:Sysparam.Ns,k)'*Heff3(:,:,k)'));
         Rate3(n) = Rate3(n)+Rate3NK(k,n);
         %% Dynamic subarray rate - Rate 4
         Heff4(:,:,k) = H(:,:,k)*S*F2k(:,:,k);
         [~,~,Veff4(:,:,k)] = svd(Heff4(:,:,k));
         Rate4NK(k,n) = log2(det(eye(Sysparam.Ns)+(Simparam.Rho/Sysparam.Ns)*Heff4(:,:,k)*Veff4(:,1:Sysparam.Ns,k)*Veff4(:,1:Sysparam.Ns,k)'*Heff4(:,:,k)'));
         Rate4(n) = Rate4(n)+Rate4NK(k,n);    
        
         %% Without beam squint compensation
         Heff5(:,:,k) = H(:,:,k)*F3;
         [~,~,Veff5(:,:,k)] = svd(Heff5(:,:,k));
         Rate5NK(k,n) = log2(det(eye(Sysparam.Ns)+(Simparam.Rho/Sysparam.Ns)*Heff5(:,:,k)*Veff5(:,1:Sysparam.Ns,k)*Veff5(:,1:Sysparam.Ns,k)'*Heff5(:,:,k)'));
         Rate5(n) = Rate5(n)+Rate5NK(k,n);
    end  
end
...
plot(NTTD,Rate1,'-r',NTTD,Rate2,'-*b',NTTD,Rate3,'-ok',NTTD,Rate4,'->')
grid on 
xlabel('M')
